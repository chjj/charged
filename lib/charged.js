...
Charged.prototype.getComponents = function(callback) {
  var self = this;
  this.getProductFamilies(function(err, results) {
    if (err) return callback(err);
    ...
    results.forEach(function(family) {
      self.getProductFamilyComponents(family.id, function(err, results) {
        if (err) return next();
...
Charged.prototype.getComponentsByFamilyHandle = function(id, callback) {
  var self = this;
  return this.getProductFamilyByHandle(id, function(err, family) {
    if (err) return callback(err);
    return self.getProductFamilyComponents(family.id, callback);
  });
};
...
Charged.prototype.createComponentDefinition = function(name, kind, options, callback) {
  if (!callback) {
    callback = options;
    options = kind;
    kind = name;
