...
Charged.webhook = function(options) {
  ...
  function getUser(event, payload, callback) {
    if (!options.getUser) return callback();
    return options.getUser(event, payload, callback);
  }
  ...
  function emit(event, payload, callback) {
    ...
    if (options.hooks[event].length === 2) {
      return options.hooks[event](payload, function(err) {
        return callback(err || null);
      });
    }
    return getUser(event, payload, function(err, user) {
