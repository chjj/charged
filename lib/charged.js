...
function HookServer(server, options) {
  ...
  options.callback = function(err, user, event, payload, data) {
    options._callback(err, user, event, payload, data);
    ...
    self.emit(user, event, payload, data);
    self.emit(user, event.replace(/_/g, '-'), payload, data);
    self.emit(user, event.replace(/_/g, ' '), payload, data);
  };
...
hooks.emit = function (event, payload, getUser, callback) {
  ...
  if (!this.hooks.hasOwnProperty(event)) {
    return callback(new Error('Bad event.'), null, event, payload, data);
  }
  ...
  if (payload.site) {
    if (payload.site.subdomain !== billing.subdomain) {
      return callback(new Error('Wrong site.'), null, event, payload, data);
    }
  }
  ...
  return getUser(data, function(err, user) {
    return err
      ? callback(err, null, event, payload, data)
      : callback(null, user, event, payload, data);
  });
};
...
hooks.middleware = function(options) {
  ...
  return function(req, res, next) {
    ...
    req.on('end', function() {
      ...
      hooks.emit(event, payload, options.getUser, options.callback);
