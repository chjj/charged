...
// Emit Hooked Event
//
var emit = function (event, payload, callback, getUser) {
...
function middleware(options) {
  ...
  if (!options.emit) {
    throw new Error('Emit function is required for charged webhook middleware.');
  }
  ...
  return function(req, res, next) {
    ...
    req.on('end', function() {
      ...
      emit(options.getUser, event, payload, function() {
...
function HookServer(server, options) {
  ...
  options.emit = function(event, payload) {
  ...
  var handler = middleware(options)
...
hooks = hooks(server, { siteKey: 'foo', getUser: function() {} });
